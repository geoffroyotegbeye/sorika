...
import { Grid3x3, Box, Layers, Eye, EyeOff } from 'lucide-react';
...
export function LayoutSection({ styles, onStyleChange }: LayoutSectionProps) {
  const { currentBreakpoint } = useEditorStore();
  ...
  const getBreakpointLabel = () => {
    if (currentBreakpoint === 'desktop') return 'Desktop';
    if (currentBreakpoint === 'tablet') return 'Tablet';
    return 'Mobile';
  };
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg">
        <div className="flex items-center justify-between mb-2">
          <Label className="text-sm font-semibold">Visibilité sur {getBreakpointLabel()}</Label>
          {isHidden ? (
        ...
        <Button
          ...
          size="sm"
          onClick={() => onStyleChange('display', isHidden ? 'block' : 'none')}
          className="w-full"
        >
          {isHidden ? (
            <>
              <EyeOff className="h-4 w-4 mr-2" />
              Masqué sur {getBreakpointLabel()}
            </>
          ) : (
            <>
              <Eye className="h-4 w-4 mr-2" />
              Visible sur {getBreakpointLabel()}
            </>
          )}
        </Button>
        <p className="text-xs text-slate-500 mt-2">
          Changez de breakpoint pour masquer/afficher sur d'autres écrans
        </p>
      </div>
